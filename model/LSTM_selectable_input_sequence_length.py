...
class LSTM:
    def __init__(self, is_train, config=DefaultConfig, generate=False, last_activation_function=tf.nn.softmax):
        ...
        if generate:
            ...
            self._inputs = inputs = tf.placeholder(tf.float32, [None, config.input_sequence_length, input_size],
                                                   "input_data")
        ...
        with tf.variable_scope("Cell"):
            ...
            # [batch_size, sequence_length, inputs_size] => [sequence_length, batch_size, inputs_size]
            converted_inputs = tf.transpose(inputs, [1, 0, 2])
            for i in range(config.input_sequence_length):
                output, state = cell(converted_inputs[i], state)
                outputs.append(output)
                if i == 0: tf.get_variable_scope().reuse_variables()
            for i in range(config.input_sequence_length, sequence_length):
        ...
        for param, gradient in zip(params, clipped_gradients):
            ...
            with tf.name_scope(param.name[:-2] + "/gradient"):
                tf.summary.scalar('max', tf.reduce_max(abs_gradient))
                tf.summary.scalar('min', tf.reduce_min(abs_gradient))
                tf.summary.scalar('mean', tf.reduce_mean(abs_gradient))
                tf.summary.histogram('gradient', gradient)
                tf.summary.histogram('var', param)
